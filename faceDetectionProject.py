...
unknown_image_to_draw = cv.imread(file_name)
...
for (top, right, bottom, left), face_encoding in zip(face_locations, face_encodings):
    ...
    cv.rectangle(unknown_image_to_draw, (left, top), (right, bottom), (0, 255, 0), 3)
    cv.putText(
        unknown_image_to_draw,
        name,
        (left, top - 20),
...
cv.imshow("window", unknown_image_to_draw)
cv.waitKey(0)
